...
def write_results(id, urm_test, dict, data_frame, component_attributes, encoder=None):
    ...
    filename = "../results/USER_CF/result_{0}.csv".format(id)
    ...
    param_file = open('../results/USER_CF/params_{0}.json'.format(id), mode='w')
    param_file.write(param_json)
    param_file.close()
    param_attributes_json = encoder.encode(component_attributes)
    param_attributes_file = open('../results/USER_CF/param_attributes_{0}.json'.format(id), mode='w')
    param_attributes_file.write(param_attributes_json)
    param_attributes_file.close()
    eval_file = open('../results/USER_CF/eval_{0}.json'.format(id), mode='w')
