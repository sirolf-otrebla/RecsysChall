from src.ensembles.ItemEnsemble import ItemEnsemble
...
def run_algorithm(data, parameters):
    if MODE == "SUBMIT":
        recommender = ItemEnsemble(data["URM_complete"], data["URM_test"], icm=data["ICM"], parameters=parameters)
    else:
        recommender = ItemEnsemble(data["URM_train"], data["URM_test"], icm=data["ICM"], parameters=parameters)
    recommender.fit()
...
def write_results(id, urm_test, dict, data_frame, component_attributes, encoder=None):
    ...
    filename = "../results/ItemEnsemble/result_{0}.csv".format(id)
    ...
    param_file = open('../results/ItemEnsemble/params_{0}.json'.format(id), mode='w')
    param_file.write(param_json)
    param_file.close()
    eval_file = open('../results/ItemEnsemble/eval_{0}.json'.format(id), mode='w')
...
def main():
    data = load_data()
    param_list = load_parameters('./parameters/ItemEnsemble.json')
    encoder = json.JSONEncoder()
    i = 0
