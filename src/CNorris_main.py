from src.ensembles.CNorrisEnsemble import CNorrisEnsemble
...
def run_algorithm(data, parameters):
    if MODE == "SUBMIT":
        recommender = CNorrisEnsemble(data["URM_complete"], data["URM_test"], icm=data["ICM"], parameters=parameters)
    else:
        recommender = CNorrisEnsemble(data["URM_train"], data["URM_test"], icm=data["ICM"], parameters=parameters)
    recommender.fit()
...
def write_results(id, urm_test, dict, data_frame, component_attributes, encoder=None):
    ...
    filename = "../results/CNorris/result_{0}.csv".format(id)
    ...
    param_file = open('../results/CNorris/params_{0}.json'.format(id), mode='w')
    param_file.write(param_json)
    param_file.close()
    param_attributes_json = encoder.encode(component_attributes)
    param_attributes_file = open('../results/CNorris/param_attributes_{0}.json'.format(id), mode='w')
    param_attributes_file.write(param_attributes_json)
    param_attributes_file.close()
    eval_file = open('../results/CNorris/eval_{0}.json'.format(id), mode='w')
...
def main():
    data = load_data()
    param_list = load_parameters('./parameters/CNorris.json')
    encoder = json.JSONEncoder()
    i = 0
