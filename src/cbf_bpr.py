from src.algorithms.p3alpha_new import P3ALPHANEW
...
def run_algorithm(data, parameters):
    if MODE == "SUBMIT":
        recommender = P3ALPHANEW(data["URM_complete"], data["URM_test"])
    else:
        recommender = P3ALPHANEW(data["URM_train"], data["URM_test"])
    recommender.fit()
...
def write_results(id, urm_test, dict, data_frame, component_attributes, encoder=None):
    ...
    filename = "../results/CBF_BPR/result_{0}.csv".format(id)
    ...
    param_file = open('../results/CBF_BPR/params_{0}.json'.format(id), mode='w')
    param_file.write(param_json)
    param_file.close()
    param_attributes_json = encoder.encode(component_attributes)
    param_attributes_file = open('../results/CBF_BPR/param_attributes_{0}.json'.format(id), mode='w')
    param_attributes_file.write(param_attributes_json)
    param_attributes_file.close()
    eval_file = open('../results/CBF_BPR/eval_{0}.json'.format(id), mode='w')
...
def main():
    ...
    for dict in param_list:
        ...
        eval = write_results(i, data['URM_test'], dict, result[1], result[2])
        i += 1
