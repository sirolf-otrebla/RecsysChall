...
def run_algorithm(data, parameters):
    ...
    recommender = BMussoliniEnsemble(data["URM_train"], data["URM_test"],parameters=None)
    recommender.fit()
...
def write_results(id, urm_test, dict, data_frame, encoder=None):
    if encoder is None:
...
def main():
    ...
    for dict in param_list:
        ...
        eval = write_results(i, data['URM_test'], dict, result[1], encoder)
        i += 1
